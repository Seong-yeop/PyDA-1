...
class Terms:
    ...
    def print_term(degree, factor):
        ...
        if degree == 0: # constant
            return str(factor) if factor % 1 else str(int(factor))
        else: # degree != 0
            if factor == 0:
                return '0'
            term_str = '-' if factor == -1 else '' if factor == 1 else str(factor) if factor % 1 else str(int(factor))
            if type(degree) is str: # cos sin exp
                term_str += degree
            else:
                term_str += 'x^' + (str(degree) if degree % 1 else str(int(degree))) if degree != 1 else 'x'
            return term_str
...
class WolframBeta:
    ...
    def compute(equation, x):
        ...
        """
        terms = Terms(equation)
        result = terms.compute_as_terms(float(x))
        return str(result) if result % 1 else str(int(result))
